...
void CacheBlock::hit_access(uint32_t mem_rw)
{
...
CacheSet::CacheSet(uint32_t way_count, uint32_t block_size):
...
void CacheSet::hit_access(CacheBlock **pp_block)
{
...
CacheBlock* CacheSet::find_block(uint64_t mem_tag)
{
    ...
    cout << "exist tag: " << m_p_mru_block->m_block_tag;
	p_block = m_p_mru_block;
	while(p_block)
	{
...
uint32_t CacheSet::load_new_block_in_LLC(uint64_t maddr, uint64_t mem_tag)
{
#ifdef DEBUG_CACHE_SIMULATOR
...
CacheSet* Caches::access_cache_at_level(uint64_t maddr, uint32_t level, uint32_t *hit_cache, uint64_t *mtag)
{
	//uint64_t mem_tag;
	uint32_t set_index;
	*hit_cache = 0;
...
void Caches::access_cache(uint64_t maddr, uint32_t memop)
{
	uint32_t i;
	...
	for(i = 0; i < m_level; i++)
	{
		access_cache_sets[i] = access_cache_at_level(maddr, i, &hit_cache, &mtags[i]);
...
void Caches::get_cache_addr_parts(uint64_t maddr, uint64_t *mem_tag, uint32_t *set_index, uint32_t level)
{
	uint64_t tmp = maddr;
...
int main(int argc, char *argv[])
{
	    ...
	    mtrace_gra = fopen("mem_trace_gra","w");

	    Transaction tr(1111111111, MEM_READ);
	    myCaches->access_cache(tr.GetAddress(), tr.GetType());
	    Transaction tr1(1111111111, MEM_WRITE);
	    myCaches->access_cache(tr1.GetAddress(), tr1.GetType());
