...
protected:
	...
	void get_cache_addr_parts(uint64_t maddr, uint64_t *mem_tag, uint32_t *set_index, uint32_t level);
	
	CacheSet* access_cache_at_level(uint64_t maddr, uint32_t level, uint32_t *hit_cache, uint64_t *mtag);
...
public:
	...
	void access_cache(uint64_t maddr, uint32_t mem_rw);
